...
@pytest.mark.parametrize("lla, expected", [([90 * u.deg, 0 * u.deg, 0 * u.km],
                                            np.array([90 - np.arctan2(-1, 2) * rtd, np.arcsin(3/norm)*rtd])),
                                           ([0 * u.deg, 0 * u.deg, 0 * u.km],
                                            np.array([90 - np.arctan2(3, 2) * rtd, np.arcsin(1/norm)*rtd])),
                                           ([0 * u.deg, 90 * u.deg, 0 * u.km],
                                            np.array([90 - np.arctan2(3, -1) * rtd, np.arcsin(2/norm)*rtd])),
...
#         actual = get_local_angles_via_state_propagation(x, params, epoch_i, epoch_f, n, obs_pos_lla, obs_frame)
