...
from src.interface.cleaning import convert_obs_params_from_lla_to_ecef, convert_obs_params_from_lla_to_eci, \
    convert_obs_params_from_ecef_to_eci, verify_locational_units
...
def test_convert_obs_params_from_lla_to_eci():
    ...
    output_pos = np.array([0, 0, 0])
    epoch = None
    input = ObsParams(input_pos, Frames.LLA, epoch)
    with patch(mockito.invocation.MatchingInvocation.compare, xcompare):
    ...
    expected = ObsParams(output_pos, Frames.ECI, epoch)
    actual = convert_obs_params_from_lla_to_eci(input)
    assert np.array_equal(expected.position, actual.position)
    assert expected.frame == actual.frame


def test_convert_obs_params_from_ecef_to_eci():
    input_pos = [1 * u.km, 2 * u.km, 3 * u.km]
    output_pos = np.array([0, 0, 0])
    epoch = None
    input = ObsParams(input_pos, Frames.ECEF, epoch)
    with patch(mockito.invocation.MatchingInvocation.compare, xcompare):
        when(cleaning).ecef_to_eci(input_pos, epoch).thenReturn(np.array(output_pos))
    expected = ObsParams(output_pos, Frames.ECI, epoch)
    actual = convert_obs_params_from_ecef_to_eci(input)
    assert np.array_equal(expected.position, actual.position)
    assert expected.frame == actual.frame
